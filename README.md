...
# Build the React frontend (required before first run and after frontend changes)
...
For local development with hot-reload at `http://localhost:8008`:

```bash
docker compose -f docker-compose.dev.yml up --build
```

This starts the Express API server and a Vite dev server in separate containers. Edit files in `frontend/src/` and changes appear instantly in the browser.

To run the frontend locally without Docker (requires Express running on port 3000):
...
├── docker-compose.dev.yml        # Local development (hot-reload at :8008)
├── frontend/                     # Vite + React + TypeScript source
│   ├── Dockerfile.dev
│   ├── vite.config.ts            # Builds to server/public/, proxies /api
│   ├── src/
│   │   ├── App.tsx               # Router + layout
│   │   ├── pages/                # One component per tab/route
│   │   ├── components/           # Shared UI components
│   │   ├── hooks/                # usePrinters, useDithering, auth hooks, etc.
│   │   └── types/api.ts          # TypeScript interfaces
│   └── package.json
├── server/
│   ├── Dockerfile
│   ├── package.json
│   ├── public/                   # ⚠ Build output only — do not edit directly
│   │                             #   Regenerated by: cd frontend && npm run build
│   └── src/
...
│           └── feedPoller.js     # RSS/Atom subscription poller
└── client/
    ├── package.json
    ├── .env.example
